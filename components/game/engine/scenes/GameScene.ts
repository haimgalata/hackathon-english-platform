...
export class GameScene extends Phaser.Scene {
  ...
  private handleGameOver() {
    ...
    // Save only the points earned in this stage (not the carried-over translation score)
    const sentenceOnlyScore = score - this.continueScore
    if (sentenceOnlyScore > 0) this.saveScoreToProfile(sentenceOnlyScore)
  ...
  // ── Save game score to student profile ────────────────────────────────────────

  private saveScoreToProfile(gamePts: number): void {
    if (gamePts <= 0) return
    try {
      const raw = localStorage.getItem('speaktech_student')
      if (!raw) return
      const student = JSON.parse(raw) as { id?: string }
      if (!student?.id) return
      const xpDelta = Math.max(1, Math.round(gamePts / 50))
